...
//			LOG_Info("Get Module Parma,T=%d, V=%d", ptRecvFrame->ucType, ptSendFrame->uData.lData)
		...
		{
			//设置IO状态
			eError = Set_Out_IO(ptRecvFrame->ucType, ptRecvFrame->uData.ulData);
...
//			uint16_t usOutState = 0, usInState = 0;
			...
			ptSendFrame->ucStatus = Get_In_IO_One(ptRecvFrame->ucType, &ptSendFrame->uData.ucData[0]);
//			//获取输出IO状态
...
//			
//			//发送数据
//			ptSendFrame->uData.ucData[0] = usOutState;
//			ptSendFrame->uData.ucData[1] = usOutState >> 8;;
//			ptSendFrame->uData.ucData[2] = usInState;
//			ptSendFrame->uData.ucData[3] = usInState >> 8;;
		}
		break;	
		case CMD_GET_OUTPUT_IO_STATUS: //0x37
		{
			ptSendFrame->ucStatus = Get_Out_IO_One(ptRecvFrame->ucType, &ptSendFrame->uData.ucData[0]);
		}
		break;
