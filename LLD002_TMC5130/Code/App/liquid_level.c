...
uint8_t AirSenCheckAll(struct tagAirSenPara* air_sen, uint8_t is_check)
{
	...
	switch(air_sen->AirPress.Stage)
	{
		case PLLD_STAGE_NULL:
		{
			...
			{
				if((air_sen->AbsState1_1 >= 1)
					|| (air_sen->AbsState1_2 >= 1))
				{
					//关闭P轴
					TMC_Stop(eTMC);
...
//				if(air_sen->AbsState1_5 >= 1)
//				{
//					air_sen->AbsState1_5 = 0;
//					
...
//					//检测到P电机开始吸液，向前移动（AIRPRESS_PASS_3 + 3）单位，取气压作为吸液前参考气压
//					air_sen->ABS_StartAirPress = air_sen->ChColl[PLLD_CH0_DATA].FilterData[AIRPRESS_PASS_3 + 3];
//					
//					
...
//			AirSenPara.sig1_cnt_6 = 0;
			AirSenPara.sig1_cnt_7 = 0;
			AirSenPara.sig1_cnt_8 = 0;
			AirSenPara.sig1_cnt_9 = 0;
//			AirSenPara.sig1_cnt_10 = 0;
...
//			AirSenPara.sig2_cnt_2 = 0;
			...
			AirSenPara.RakeRatio4Max = UserPara[AIR_RAKERATIO4_MAX].Value;
			AirSenPara.RakeRatio4Min = UserPara[AIR_RAKERATIO4_MIN].Value;
			...
			for(i = set_comp_num1; i > 0; i--)
			{
				...
				//1、探测气泡液面。
				//气压跳变幅度很大，说明无气泡。需要清零
				if(curve_rake_ratio > AirSenPara.RakeRatio4Max)
				{
...
//				if(curve_rake_ratio < (-AirSenPara.RakeRatio4Max))
//				{
//					Accumulation8(&AirSenPara.sig1_cnt_4);
//				}
//				else
//				{
//					AirSenPara.sig1_cnt_4 = 0;
//				}
			...
			for(i = set_comp_num1; i >= 5; i--)
			{
				...
				{
					Accumulation8(&AirSenPara.sig1_cnt_5);
				}
				else
				{
					AirSenPara.sig1_cnt_5 = 0;
...
//				}
				...
				{
					Accumulation8(&AirSenPara.sig1_cnt_7);
				}
				else if(curve_rake_ratio < (-AirSenPara.RakeRatio1Min))
				{
					Accumulation8(&AirSenPara.sig1_cnt_8);
				}
				...
				{
					Accumulation8(&AirSenPara.sig1_cnt_9);
				}
				else
				{
					AirSenPara.sig1_cnt_9 = 0;
			...
			AirSenPressState2(&AirSenPara.AbsState1_2, AirSenPara.sig1_cnt_2, AIRPRESS_PASS_1);
			
			//(2)识别气泡。气压跳变幅度很大，说明无气泡。
			AirSenPressState2(&AirSenPara.AbsState1_3, AirSenPara.sig1_cnt_3, AIRPRESS_PASS_1);
			
			//(3)启动吸气
			AirSenPressState2(&AirSenPara.AbsState1_5, AirSenPara.sig1_cnt_5, AIRPRESS_PASS_2);
			
			//(4)识别气泡。
			AirSenPressState1(&AirSenPara.AbsState1_7, AirSenPara.sig1_cnt_7);
			AirSenPressState1(&AirSenPara.AbsState1_8, AirSenPara.sig1_cnt_8);
			AirSenPressState1(&AirSenPara.AbsState1_9, AirSenPara.sig1_cnt_9);
...
void AirSenInit(void)
{
	...
	//吸液大斜率，探测液面
	UserPara[AIR_RAKERATIO1_MAX].Value = 40;
	UserPara[AIR_RAKERATIO1_MIN].Value = 25;
	...
	//打开探测气泡功能，但液面无气泡
	UserPara[AIR_RAKERATIO4_MAX].Value = 320;
	UserPara[AIR_RAKERATIO4_MIN].Value = 10;
...
uint8_t WriteAirLLDPara1(uint8_t *buf, uint8_t num)
{
	...
	{
		index = AIR_RAKERATIO4_MAX;
	}
	else if(12 == num)
	{
		index = AIR_RAKERATIO4_MIN;
	}
